...
module.exports = function (collectionName, resultKey) {
  ...
  this.populate = function (crudRelation, attribute, fields) {
    return function (req, res, next) {
      ...
      var obj = utils.getNestedObject(req[resultKey], attribute);
      var docs = [];

      if (Array.isArray(obj)) {
        populateArray(crudRelation, obj, fields).then(function (docs) {
          utils.setNestedObject(req[resultKey], attribute, docs);
          next();
        }, function (err) {
          logger.error(err);
          res.status(500).json({
        ...
        populateItem(crudRelation, obj, fields).then(function (doc) {
          utils.setNestedObject(req[resultKey], attribute, doc);
          next();
        }, function (err) {
          logger.error(err);
          res.status(500).json({
